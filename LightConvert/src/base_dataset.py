...
class BaseDataset(object):
    ...
    def GeneralProcess(self, processed_data):
        # split the category
        processed_data.iloc[:,2] = processed_data.iloc[:,2].apply(
            lambda row: ', '.join(row.split('|'))
        )
        # replace `(no genres listed)` with `unknown`
        processed_data.iloc[:, 2] = processed_data.iloc[:, 2].replace('(no genres listed)', 'unknown')
        # get title (release_year)
        title_time = processed_data.iloc[:,1].apply(lambda row: row.rsplit('(',1))
        processed_data.iloc[:,1] = title_time.apply(lambda row: row[0].strip())
        # some movies don't include release_year
        time = title_time.apply(lambda x: x[1].replace(')', '') if len(x) > 1 else None)
        processed_data.insert(2, 'release_year', pd.Series(time))         
